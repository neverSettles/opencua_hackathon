"""
Run T3 (REI hiking boot constraint search) end-to-end on Kernel + a pluggable
model backend. Mirrors run_t2_spike.py.

Reads:
  - tasks/T3_rei_hiking_boots.md       (extracts the fenced "Agent prompt" block)
  - ground_truth/T3_ground_truth.json

Usage:
  uv run python -u run_t3_spike.py --model anthropic --anthropic-model claude-opus-4-7
  uv run python -u run_t3_spike.py --model openai --openai-model gpt-5.5
"""
...
from scripts.scoring import score_t3  # noqa: E402

VIEWPORT_W = 1280
VIEWPORT_H = 800
START_URL = "https://www.rei.com/c/mens-hiking-boots"
TASK_SPEC_PATH = REPO_ROOT / "tasks" / "T3_rei_hiking_boots.md"
GT_PATH = REPO_ROOT / "ground_truth" / "T3_ground_truth.json"
WORLD_ID = "rei_v1"
TASK_ID = "T3-001"
...
def _extract_agent_prompt(spec_md: str) -> str:
    ...
    if not m:
        raise RuntimeError("Could not find fenced agent prompt block in T3 spec")
...
def main() -> int:
    ...
    run_id = f"t3_{adapter.name}_{_now_id()}"
    ...
    print(f"Mode: T3 REI hiking boots | max_steps={args.max_steps}")

    kernel = Kernel(api_key=os.environ["KERNEL_API_KEY"])
    print(f"Creating Kernel browser ...")
    browser = kernel.browsers.create(
        stealth=True, headless=False, timeout_seconds=1800,
    ...
    if gt is not None and parsed is not None:
        # Build a thin trajectory list for the filter_engagement check.
        traj_actions: list[dict] = []
        try:
            with traj_path.open() as f:
                for line in f:
                    rec = json.loads(line)
                    traj_actions.append({
                        "target": rec.get("text") or "",
                        "url": rec.get("url") or "",
                    })
        except Exception:
            pass
        s = score_t3(parsed, gt, traj_actions)
        score_dict = s
