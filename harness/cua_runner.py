"""Vendored CUA runner from tzafon/lightcone (examples/harness/runner.py).

Source: https://github.com/tzafon/lightcone (Apache 2.0)
Why vendored: their runner is decoupled from the computer backend (just calls
methods on a `computer` object), so by giving it a Kernel-backed adapter we get
their circuit breaker, click-before-type, step-budget warnings, and hooks for
free instead of reinventing them.

Local edits vs upstream: none (kept verbatim except this header).
"""
    ...
    def run(
        ...
        *,
        ...
        with context as session:
            ...
            no_action_streak = 0
            MAX_NO_ACTION_STREAK = 3

            for step in range(1, self.config.max_steps + 1):
                self._emit_messages(step, response, computer_id)
                computer_call = self._find_computer_call(response)
                if computer_call is None:
                    # Northstar narrated without emitting an action.
                    # If the message contains a JSON object, that's likely the final
                    # answer - treat as done. Otherwise nudge and retry up to N times.
                    final_message = self._collect_message_text(response) or ""
                    looks_like_final = (
                        "{" in final_message and "}" in final_message
                    ) or any(kw in final_message.lower() for kw in ("complete", "finished", "done"))
                    no_action_streak += 1

                    if looks_like_final or no_action_streak > MAX_NO_ACTION_STREAK:
                        result = RunResult(
                    ...
                    self._log(f"[{step}] no_action (streak {no_action_streak}); nudging")
                    self._trace("no_action_retry", {"step": step, "streak": no_action_streak})
                                ...
                                "content": [
                                    {
                                        "type": "input_text",
                                        "text": (
                                            "You did not emit a computer action. "
                                            "Look at the current screenshot and emit the next "
                                            "action. Do not narrate; act."
                                        ),
                                    },
                ...
                # Keep up to 12 sigs so we can detect cycles up to period 4.
                ...
                if (
                    ...
                    and len(set(recent_actions[-self.config.circuit_breaker_threshold:])) == 1
                ):
                    cycle_kind = "identical"
                else:
                    # Cycle of period 2..4 repeated >= 2 times.
                    for period in (2, 3, 4):
                        if len(recent_actions) >= 2 * period:
                            tail = recent_actions[-2 * period :]
                            if tail[:period] == tail[period:]:
                                cycle_kind = f"period-{period}"
                                break

                if cycle_kind is not None:
                    self._log(f"[{step}] {cycle_kind} cycle detected, redirecting")
                    self._trace(
                        "circuit_breaker",
                        {"step": step, "kind": cycle_kind, "recent": recent_actions[-8:]},
                    )
                    session.wait(2)
                    screenshot_url = self._capture_screenshot(session)
                    response = self._create_follow_up_response(
                        ...
                        screenshot_url=screenshot_url,
                        message=(
                            "You are stuck in a loop, repeating the same sequence of actions. "
                            "This approach is NOT working. Stop what you're doing and try a "
                            "completely different approach. If the page is unchanged, the prior "
                            "action did nothing — DO NOT click the browser back button if you've "
                            "just navigated to the start page; instead, work directly on the "
                            "current page. If the task is blocked, emit an `answer` action with "
                            "your best partial result as JSON."
                        ),
                    )
    ...
    def _capture_screenshot(self, computer: Any) -> str | None:
        ...
        self._trace(
            "screenshot_captured",
            {
                ...
                "screenshot_url": screenshot_url[:120] if screenshot_url else None,
            },
        )
    ...
    def _emit_messages(self, step: int, response: Any, computer_id: str) -> None:
        for item in getattr(response, "output", None) or []:
            ...
            for block in item.content or []:
                ...
                self._log(f"[{step}] model={block.text[:200]}")
